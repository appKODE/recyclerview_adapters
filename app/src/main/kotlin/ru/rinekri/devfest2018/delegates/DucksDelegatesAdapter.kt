...
import ru.rinekri.devfest2018.models.items.DisplayableItem
import ru.rinekri.devfest2018.models.items.DuckItem
...
private class RubberDuckDelegate : AbsListItemAdapterDelegate<DuckItem, DisplayableItem, RubberDuckDelegate.ViewHolder>() {

  override fun isForViewType(item: DisplayableItem, items: List<DisplayableItem>, position: Int): Boolean {
    return item is DuckItem
  }
  ...
  override fun onBindViewHolder(item: DuckItem, viewHolder: ViewHolder, payloads: List<Any>) {
    viewHolder.apply {
      val context = itemView.context
      rubberDuckImage.showIcon(item.icon)
      rubberDuckCounts.adapter = item.duckCountsAdapter
